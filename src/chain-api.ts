...
export default class ChainApi implements IChainApi {
  ...
  holders(opts?: Partial<ITableRequest>): Promise<IHolder[]> {
    return this.apiRequest<ITableResponse, ITableRequest>("get_table_rows", {
      ...
      let holders = responce.rows.map(name =>
        this.eos.fc.fromBuffer("name", name)
      );

      if (this.options.ignoreHolders && this.options.ignoreHolders.length) {
        holders = holders.filter(
          name => this.options.ignoreHolders.indexOf(name) === -1
        );
      }
  ...
  balance(holder: IHolder, opts?: Partial<ITableRequest>): Promise<IBalance> {
    ...
    return this.apiRequest<ITableResponse, ITableRequest>("get_table_rows", {
      ...
      .then(responce => {
        return responce.rows.map(
          name => this.eos.fc.fromBuffer("asset", name) as string
        );
      })
      .then(
        assets =>
          assets.find(asset => asset.endsWith(this.options.symbol)) as string
      )
      .then(asset => ({
        holder,
  ...
  private apiRequest<TResponse, TRequest>(
    ...
    return new Promise<TResponse>((resolve, reject) => {
      ...
      nodes.map(endpoint => {
        this.logger.debug(
          `Calling ${endpoint} for ${action} data with: ${JSON.stringify(data)}`
        );
        return this.axios
